...
class MOptionTree extends FunSuite with Matchers
{
  ...
  test("exe 4.4 sequence all ok ")
  {
      val v = sequence(as)
      v shouldEqual MSome(List(1,2,3))
  }

  test("exe 4.4 sequence met one none")
  {
      val bs = as :+ MNone
      val v = sequence(bs)
      v shouldEqual MNone
  ...
  test("exe 4.4.2 sequenceByTraverse all ok ")
  {
      val v = sequenceByT(as)
      v shouldEqual MSome(List(1,2,3))
  }

  test("exe 4.4.2 sequenceByTraverse met one none")
  {
      val bs = as :+ MNone
      val v = sequenceByT(bs)
      v shouldEqual MNone
  }
